...
void AOF::computeBlock1(long s, size_t k) {
    P[s] = computeProbabilityDensityN(Omega[s], m_sampleY[s], Mu[s], Phi[s]); // НАДО ПОПРАВИТЬ!
    K[s] = Delta[s]*Pinv(Phi[s]);
    Sigma[s] = Lambda[s] + K[s]*(m_sampleY[s] - Mu[s]);
    Upsilon[s] = Psi[s] - K[s]*Delta[s].transpose();
}

void AOF::computeBlock2(long s, size_t k) {
    Vector resZ = Vector::Zero(Sigma[s].size());
    Vector mult = Vector::Zero(Sigma[s].size());
    for (int i = 0; i < m_task->countI; i++) {
        mult = P[s][i]*Sigma[s][i];
        resZ += mult;
    }
