...
enum PlottingHint {
    ...
    ///<                This is set by default to prevent the plot from freezing on fast consecutive replots (e.g. user
...
extern const QMetaObject staticMetaObject; // in moc-run we create a static meta object for QCP "fake" object. This line
...
public:
    ...
    */
    enum ScatterShape {
        ...
        ,
        ssDot ///< \enumimage{ssDot.png} a single pixel (use \ref ssDisc or \ref ssCircle if you want a round shape with
              /// a certain radius)
        ,
...
void QCPDataContainer<DataType>::removeBefore(double sortKey)
{
    ...
    mPreallocSize += itEnd - it; // don't actually delete, just add it to the preallocated block (if it gets too large,
...
public:
    ...
    */
    enum PositionType {
        ...
        ///< left corner of the viewport/widget. setCoords(1, 1) will be at the bottom right corner, setCoords(0.5, 0)
        /// will be horizontally centered and
        ...
        ///< left corner of the axis rect. setCoords(1, 1) will be at the bottom right corner, setCoords(0.5, 0) will be
        /// horizontally centered and
        ///< vertically at the top of the axis rect, etc. You can also go beyond the axis rect by providing negative
        /// coordinates or coordinates larger than 1.
        ,
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF &pos, bool onlySelectable, QVariant *details) const
{
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
        ...
        if (keyRange.contains(mainKey) && valueRange.contains(mainValue)) // make sure data point is inside visible
